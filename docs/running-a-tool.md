...
The driver injects only the flags shown above (account, array, partition, gres, logs) plus the positionals. **Resource directives — wall time, memory, `cpus-per-task`, job name — are not CLI flags; they live in the tool's own `.sbatch` as `#SBATCH` lines.** So "how much" a task gets is a property of the tool script, while "how many, where, and under which account" is what you set at `sapia run` time. To change a tool's resources, edit (or [fork](writing-a-tool.md#customizing-bundled-tools)) its `.sbatch` — see [Writing a `.sbatch`](writing-a-tool.md#writing-a-sbatch).
