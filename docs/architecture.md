...
Tables are stored as `.tsv` by default, but the [`DataManager`](#the-drivers) handles them uniformly as pandas DataFrames through a pluggable I/O backend, so the on-disk format is not load-bearing.
...
├── table1_seqs.tsv           # a child table (another generation)
...
1. User calls `sapia run <tool>` against an input table (`table0.tsv`) inside a `run_dir`. The driver opens the input table, reserves the destination table (see below), keeps the rows that are *ready* and hands them to the tool's `build_manifest_fn`, which writes a tab-separated **manifest** — one line per array task. 
2. The driver then submits a SLURM array job whose `tool.sbatch` consumes that manifest. Each task cuts its own fields out of its manifest line and writes results under `run_dir/<output_table>/<tool>/` — keyed by the destination table reserved in step 1. The `sapia run` flags that shape this array (concurrency, partitions, GPUs, filtering) are documented in [Running a tool](running-a-tool.md).
3. User calls `sapia collect` against the output table (`table1.tsv`) inside the `run_dir`. The driver invokes the tool's `collect_fn`, which reads those on-disk outputs, writes rows into the destination table and updates the registry. The manifest is transient scaffolding; the **table is the durable record.**

This is the second principle in action: the tool contributes only `build_manifest_fn` for `sapia run` and `collect_fn` for `sapia collect` (plus its `.sbatch`). Everything else — filtering, resume-on-rerun, SLURM submission, lineage stamping, table writes — is the shared driver. 

### The output table is derived, not chosen

You never name a tool's output table — the driver derives it in **phase 1**, the moment you call `sapia run`, from the input table plus the tool's `action`:

- an **`update`** tool writes back into the same table it read;
- a **`create`** tool reserves that table's **next generation** — a fresh child `table<gen+1>` linked to its parent — and registers it in the `_registry` immediately. (With no input table, a `create` tool starts a fresh root, `table0`.)

This is why, by the time tasks run in **phase 2**, their outputs already land under `run_dir/<output_table>/<tool>/`: the destination's identity precedes its rows. `sapia collect` in **phase 3** then fills the very table that `run` reserved.
